...
namespace xt
{
     ...
     * \code{cpp}
     * xt::xarray<float> a = {1, 10, -10, 123};
     * std::cout << xt::partition(a, 0) << std::endl; // {2, 0, 3, 1} the correct entry at index 0
     * std::cout << xt::partition(a, 3) << std::endl; // {0, 1, 2, 3} the correct entry at index 3
     * std::cout << xt::partition(a, {0, 3}) << std::endl; // {2, 0, 1, 3} the correct entries at index 0 and 3
     * \endcode
     *
     * @param e input xexpression
     * @param kth_container a container of ``indices`` that should contain the correctly sorted value
    ...
    {
        return argpartition(e, xtl::forward_sequence<std::array<std::size_t, N>>(kth_container), tag);
    }
    ...
    {
        return argpartition(e, xtl::forward_sequence<std::array<std::size_t, N>>(kth_container), axis);
    }
